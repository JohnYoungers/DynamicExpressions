...
namespace DynamicExpressions.Linq
{
    internal class ExpressionParser
    {
        ...
        {
            keywords = keywords ?? CreateKeywords();
...
#pragma warning restore 0219
        ...
        {
            Expression left = ParseLogicalIn();
            while (token.id == TokenId.DoubleAmphersand || TokenIdentifierIs("and"))
            {
                Token op = token;
                NextToken();
                Expression right = ParseLogicalIn();
                CheckAndPromoteOperands(typeof(ILogicalSignatures), op.text, ref left, ref right, op.pos);
        ...
        Expression ParseIntegerLiteral()
        {
            ...
            if (tokenText[0] != '-')
            {
                if (!UInt64.TryParse(tokenText, NumberStyles.Integer, NumberFormatInfo.InvariantInfo, out ulong value))
                    throw ParseError(Res.InvalidIntegerLiteral, tokenText);
                NextToken();
                if (value <= (ulong)Int32.MaxValue) return CreateLiteral((int)value, tokenText);
                if (value <= (ulong)UInt32.MaxValue) return CreateLiteral((uint)value, tokenText);
                if (value <= (ulong)Int64.MaxValue) return CreateLiteral((long)value, tokenText);
                return CreateLiteral(value, tokenText);
            }
            else
            {
                if (!Int64.TryParse(tokenText, NumberStyles.Integer, NumberFormatInfo.InvariantInfo, out long value))
                    throw ParseError(Res.InvalidIntegerLiteral, tokenText);
                NextToken();
        ...
        Expression ParseRealLiteral()
        {
            ...
            if (last == 'F' || last == 'f')
            {
                if (float.TryParse(token.text.Substring(0, token.text.Length - 1), NumberStyles.Float | NumberStyles.AllowThousands, NumberFormatInfo.InvariantInfo, out float f)) value = f;
            }
            else
            {
                if (double.TryParse(token.text, NumberStyles.Float | NumberStyles.AllowThousands, NumberFormatInfo.InvariantInfo, out double d)) value = d;
            }
            if (value == null) throw ParseError(Res.InvalidRealLiteral, token.text);
            NextToken();
        ...
        Expression ParseIdentifier()
        {
            ...
            {
                if (value is Type t) return ParseTypeAccess(t);
        ...
        static Dictionary<string, object> CreateKeywords()
        {
            ...
            foreach (Type type in Predefined.Types) d.Add(type.Name, type);
            return d;
        }
    }
}
